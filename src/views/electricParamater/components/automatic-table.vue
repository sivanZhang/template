        ...
        <div class="el-btn-list-row">
          ...
          <el-row class="row-bg">
            ...
            <el-col :sm="7" class="col-bg">
              ...
              class="button-style" 
              @click="addAdjustActive()">自动调整有功数据</el-button>
            </el-col>
            <el-col :sm="7" class="col-bg">
              <el-button
                type="primary"
...
export default {
  ...
  methods: {
    ...
    addAdjustActive() {
      ...
      // 值为adjustData，索引为adjustDataIndex
        ...
        for(var i=0;i<this.activeData.length;i++){
          if((Math.abs(this.activeData[i-1])-Math.abs(this.activeData[i]))>=0
          &&(Math.abs(this.activeData[i+1])-Math.abs(this.activeData[i]))>=0){
        ...
        if(subData>0){
         ...
         newactiveDataList.push(newactiveDataList[0])
          ...
          // // 下移subData个位
          reverse(this.activeData,0,length-1);
          reverse(this.activeData,0,Math.abs(subData)-1);
           newactiveDataList=reverse(this.activeData,Math.abs(subData),length-1);
    ...
    // 调整有功扭矩计算
    addActiveTorque() {
      // let rush_times = this.parameter[0].rush_times
      let rush_times =5.86
      // 获取后端返回的冲次
      ...
      this.activeDataList.map((item, index) => {
        ...
        // activeTorqueList.push(Computed(item).toFixed(9));
      });
    ...
    getBalanceTorque(){
      ...
      if(this.wellpoint == 145){
        ...
        this.angle145.map((item, index)=>{
          ...
          // balanceTorqueList.push(Computed(item).toFixed(9));
        })
      }else{
         balanceTorqueList = []
    ...
    symmetry(sum,firstTorqueIndex){
      ...
      if(this.wellpoint == 145){
         for(var i=0;i<=symmetryIndexEnde;i++){
            ...
            // this.adjustBalTorqueList[i]=computed(this.angle145[i],this.JD1)
          }
      }else{
        for(var i=0;i<=symmetryIndexEnde;i++){
            this.adjustBalTorqueList[i]=Math.round(computed(this.angle200[i],this.JD1) * 1000000000) / 1000000000
            // this.adjustBalTorqueList[i]=computed(this.angle200[i],this.JD1)
          }
      }
    },
    firstToSecond(sum,firstTorqueIndex,secondTorqueIndex){
      ...
      for(var i=start,j=0;i<length;i++){
          if(this.wellpoint == 145){
            ...
            // this.adjustBalTorqueList[i]=computed(j,this.angle145[i],this.JD1,this.JDsub,count)
          }else{
            ++j
            this.adjustBalTorqueList[i]=Math.round(computed(j,this.angle200[i],this.JD1,this.JDsub,count) * 1000000000) / 1000000000
            // this.adjustBalTorqueList[i]=computed(j,this.angle200[i],this.JD1,this.JDsub,count)
          }
      }      
    },
    secondToEnd(sum,secondTorqueIndex){
      ...
      if(this.wellpoint == 145){
         ...
         for(var i=start,j=0;i<145;i++){
            ...
            // this.adjustBalTorqueList[i]=computed(j,this.angle145[i],this.JD2,this.JDsub,count)
         }
      }else{
        count =200 -start;
        for(var i=start,j=0;i<200;i++){
            ...
            // this.adjustBalTorqueList[i]=computed(j,this.angle200[i],this.JD2,this.JDsub,count)
         }
      }
    },
    // 一个冲次调整平衡扭矩计算
    ...
    adjustBalanceActive() {
      ...
      for(var i=0;i<length;i++){
        ...
        // this.balanceActiveList[i]=Computed(this.activeTorqueList[i],this.adjustBalTorqueList[i])
      }
      this.realdata.map((item, index) => {
    ...
    changeElectricParam() {
      ...
      for(var i=0;i<this.realdata.length;i++){
        ...
        // this.electricParamList[i]=Computed(this.balanceActiveList[i],this.torque[i])
      }
      this.realdata.map((item, index) => {

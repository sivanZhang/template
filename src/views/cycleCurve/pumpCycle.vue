<template>
  <div id="diagQuery">
    <div class="diag-title">
      <div class="title-text">{{wellDetail.well_type == 0?'抽油机周波曲线分析界面':'螺杆泵周波曲线分析界面'}}</div>
    </div>
        ...
        <el-col :lg="13" :sm="13">
          ...
          >
            <el-tabs type="card" v-model="activeName" @tab-click="handleClick">
              ...
              <el-tab-pane label="历史曲线" name="third"></el-tab-pane>
            </el-tabs>
          </el-card>
        </el-col>
      </el-row>
...
export default {
  data() {
    return {
      name:1,//用来在子组件里面判断是周波曲线分析(1)还是诊断处理界面(0)
      wellPoint: 145,
  ...
  methods: {
    ...
    //获取井的详情
    getWellDetails() {
      getWellDetail({wellid: this.wellName, welldetail: ""}).then(({ data }) => {
